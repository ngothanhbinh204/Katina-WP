...
/**
 * Set custom posts per page for project-category taxonomy
...
function custom_du_an_posts_per_page($query) {
    ...
    if (!is_admin() && $query->is_main_query()) {
        // Check if this is a project-category taxonomy query by checking query vars
        $is_du_an_category = false;
        $term_slug = '';
        
        // Check if taxonomy query var is set
        if (isset($query->query_vars['taxonomy']) && $query->query_vars['taxonomy'] === 'project-category') {
        ...
        elseif (isset($query->query_vars['project-category'])) {
            $is_du_an_category = true;
            $term_slug = $query->query_vars['project-category'];
        }
        
        if ($is_du_an_category && $term_slug) {
            // Get term by slug
            $term = get_term_by('slug', $term_slug, 'project-category');
            ...
            if ($term && !is_wp_error($term)) {
                ...
                if($highlighted_projects && !empty($highlighted_projects)) {
                    ...
                    foreach($highlighted_projects as $project) {
                        ...
                        if($post_obj && $post_obj->post_type === 'project' && $post_id) {
...
function custom_du_an_posts_per_page_fallback($query) {
    ...
    if (!is_admin() && $query->is_main_query()) {
        ...
        // Check if this is a project-category term
        if ($term && !is_wp_error($term) && isset($term->taxonomy) && $term->taxonomy === 'project-category') {
            $highlighted_projects = get_field('highlight_project', $term);
            if(!$highlighted_projects || (is_array($highlighted_projects) && empty($highlighted_projects))) {
                $highlighted_projects = get_field('highlight_project', $term->taxonomy . '_' . $term->term_id);
            }
...
function my_add_cpt_and_taxonomy_pagination_rules() {
    ...
    // This is critical for post types like 'project' which has has_archive => false
...
/* ----------------------------
 * Debug: Log rewrite rules for project-category (remove after testing)
...
add_action( 'init', function() {
    if ( current_user_can( 'manage_options' ) && isset( $_GET['debug_rewrite'] ) ) {
        ...
        foreach ( $rules as $pattern => $query ) {
            if ( strpos( $pattern, 'project-category' ) !== false || strpos( $query, 'project-category' ) !== false ) {
